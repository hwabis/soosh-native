...
void Server::sendMessage(std::shared_ptr<ip::tcp::socket> socket,
      ...
      *socket, boost::asio::buffer(*buffer),
      [this, socket, buffer](const boost::system::error_code &ec,
        ...
        if (!ec) {
          ...
          timer->async_wait([this, socket,
                             timer](const boost::system::error_code &ec) {
            if (!ec) {
              sendMessage(socket,
                          "Another message from the server after a delay.\n");
            }
          });
        } else {
          if (ec == boost::asio::error::connection_reset ||
              ec == boost::asio::error::eof) {
            print("Client disconnected. No further messages will be sent to "
                  "this client.");
          } else {
